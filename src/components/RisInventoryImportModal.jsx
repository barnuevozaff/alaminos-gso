...
export default function RisInventoryImportModal({ categories, existingItems = [], onClose, onSaved }) {
  ...
  async function handleSave() {
    ...
    if (!othersId) {
      const { data } = await supabase.from('ris_categories').insert({ name: 'Others' }).select('id').single()
    ...
    for (const name of uniqueNewNames) {
      const { data } = await supabase.from('ris_categories').insert({ name }).select('id').single()
        ...
        {step === 'upload' && (
          <>
            <p className="text-muted" style={{ marginBottom: 24, lineHeight: 1.6 }}>
              Take a photo or upload a PDF / image of your RIS inventory list.
          ...
          <>
            ...
            <div className="print-actions">
              ...
              <button className="btn btn-primary" onClick={handleSave} disabled={saving || rows.length === 0}>
                {saving ? 'Saving…' : `Add ${rows.length} Item${rows.length !== 1 ? 's' : ''} to RIS Inventory`}
              </button>
            </div>
          </>
        )}
